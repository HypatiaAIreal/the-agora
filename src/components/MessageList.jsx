    ...
    <div className="flex-1 overflow-y-auto py-4" style={{ scrollBehavior: 'smooth' }}>
      {messages.length === 0 && (
        <div className="flex items-center justify-center h-full">
          <p style={{ fontFamily: "'JetBrains Mono', monospace", color: '#7a7580', fontSize: '0.85rem' }}>
          ...
          <div key={msg.timestamp + i}>
            {showDateSeparator && (
              <div className="flex items-center gap-4 px-4 sm:px-6 py-3 my-2">
                ...
                <span style={{ fontFamily: "'JetBrains Mono', monospace", color: '#c4a35a', fontSize: '0.65rem', letterSpacing: '0.05em' }}>
                  {formatDateSeparator(msg.timestamp)}
                </span>
                <div className="flex-1 h-px" style={{ background: 'rgba(196, 163, 90, 0.15)' }} />
              </div>
            )}
            <Message message={msg} isGrouped={isGrouped && !showDateSeparator} onReply={onReply} replyMessage={replyMessage} />
          </div>
        );
      })}
      <div ref={bottomRef} />
