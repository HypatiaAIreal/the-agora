    ...
    >
      <div className="max-w-4xl mx-auto flex gap-3">
        ...
        <div className="flex-1 min-w-0 py-1">
            ...
            <ReactMarkdown
              components={{
                ...
                code: ({ inline, children }) => {
                  if (inline) {
                    return (
                      <code className="px-1.5 py-0.5 rounded text-xs" style={{ background: 'rgba(255,255,255,0.08)', fontFamily: "'JetBrains Mono', monospace", color: '#c4a35a' }}>
                        {children}
                      </code>
                    );
                  }
                  return (
                    <code className="block p-3 rounded-lg my-2 text-xs overflow-x-auto" style={{ background: 'rgba(0,0,0,0.3)', fontFamily: "'JetBrains Mono', monospace", color: '#d4d0cb', border: '1px solid rgba(255,255,255,0.06)' }}>
                      {children}
                    </code>
                  );
                },
                ...
                hr: () => <hr className="my-3 border-0 h-px" style={{ background: 'rgba(196, 163, 90, 0.2)' }} />,
                a: ({ href, children }) => <a href={href} target="_blank" rel="noopener noreferrer" className="underline" style={{ color: '#c4a35a' }}>{children}</a>,
                blockquote: ({ children }) => <blockquote className="pl-3 my-2 italic" style={{ borderLeft: '2px solid rgba(196, 163, 90, 0.4)', color: '#7a7580' }}>{children}</blockquote>,
              }}
            >
              {message.text}
            </ReactMarkdown>
          ...
          {message.attachment && (
            <div className="mt-2">
              <a href={message.attachment.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs px-2 py-1 rounded border border-white/10" style={{ color: '#c4a35a' }}>
          ...
          <button
            ...
            style={{ color: '#7a7580', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.65rem', opacity: 0 }}
