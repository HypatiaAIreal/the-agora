    ...
    <div className="flex items-center justify-center min-h-screen px-4" style={{ background: '#08080d' }}>
      <div className="w-full max-w-sm rounded-xl p-8"
        ...
        <div className="text-center mb-8">
          ...
          {mode === 'register' ? (
            <>
              <h1 className="text-xl font-light" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#d4d0cb' }}>
                This space was built for you.
              </h1>
              <p className="text-xs mt-2" style={{ fontFamily: "'DM Sans', sans-serif", color: '#7a7580', lineHeight: 1.5 }}>
                Choose how you want to be called.<br />Only you will have this access.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="text-xs mt-2" style={{ fontFamily: "'DM Sans', sans-serif", color: '#7a7580' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="text" value={username} onChange={(e) => setUsername(e.target.value)}
            ...
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#d4d0cb', fontFamily: "'DM Sans', sans-serif" }} />
          <input type="password" value={password} onChange={(e) => setPassword(e.target.value)}
            placeholder="Your secret key" autoComplete={mode === 'register' ? 'new-password' : 'current-password'}
            className="w-full px-4 py-2.5 rounded-lg text-sm outline-none"
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: '#d4d0cb', fontFamily: "'DM Sans', sans-serif" }} />
          ...
          <button type="submit" disabled={submitting || !username || !password}
            ...
            style={{
              background: 'rgba(38, 166, 154, 0.2)', border: '1px solid rgba(38, 166, 154, 0.3)',
              color: '#26a69a', fontFamily: "'DM Sans', sans-serif",
              opacity: (!username || !password) ? 0.5 : 1, cursor: submitting ? 'wait' : 'pointer',
            }}>
        ...
        {mode === 'register' && (
          <p className="text-center mt-6 text-xs" style={{ color: '#7a758044', fontFamily: "'DM Sans', sans-serif", fontSize: '0.65rem' }}>
            Your conversations are private. Only you can access this space.
          </p>
        )}
        {mode === 'login' && (
          <button onClick={() => setMode('register')} className="block mx-auto mt-4 text-xs"
...
function ContextPanel({ items, onDelete, open, onClose }) {
  return (
    <>
      ...
      <aside className={`
        fixed right-0 top-0 bottom-0 w-80 z-40 transition-transform lg:static lg:z-auto
        ${open ? 'translate-x-0' : 'translate-x-full'} lg:translate-x-0
        flex flex-col overflow-hidden border-l border-white/5
      `} style={{ background: '#12121a' }}>
        <div className="px-4 py-4 border-b border-white/5">
          <h2 className="text-sm" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#26a69a' }}>
            What Athena remembers about you
          </h2>
          <p className="text-xs mt-0.5" style={{ color: '#7a758066', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.55rem' }}>
            You can delete anything here
          </p>
        </div>
        <div className="flex-1 overflow-y-auto p-3 space-y-2">
          {(!items || items.length === 0) && (
            <p className="text-xs text-center py-4" style={{ color: '#7a758044' }}>No memories yet</p>
          )}
          {(items || []).map((item, i) => {
              ...
              <div key={item.id || i} className="rounded-lg p-3" style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.05)' }}>
                <div className="flex items-center justify-between mb-1.5">
                  <span className="text-xs px-2 py-0.5 rounded-full"
                    style={{ background: `${catColor}18`, color: catColor, fontFamily: "'JetBrains Mono', monospace", fontSize: '0.55rem' }}>
                    {item.category || 'note'}
                  </span>
                  <button onClick={() => onDelete(item.id)} title="Delete"
                    className="text-xs p-1 rounded transition-colors" style={{ color: '#7a758044', fontSize: '0.7rem' }}>
                    {'\ud83d\uddd1\ufe0f'}
                  </button>
                </div>
                <p className="text-xs leading-relaxed" style={{ color: '#d4d0cb', fontFamily: "'DM Sans', sans-serif", fontSize: '0.8rem' }}>
...
function FeedbackRow({ messageTimestamp, onSend }) {
  ...
  const handleRate = (stars) => {
    setRating(stars);
    onSend({ type: 'rating', stars, messageTimestamp });
    setSent(true);
  };
  const handleThumbsDown = () => {
    onSend({ type: 'thumbs_down', messageTimestamp });
    setSent(true);
  };
  ...
  return (
    <div className="flex items-center gap-3 mt-1.5">
      <div className="flex gap-0.5">
        {[1,2,3,4,5].map((s) => (
          <button key={s} onClick={() => handleRate(s)} className="text-sm transition-colors"
            style={{ color: s <= rating ? '#c4a35a' : '#7a758033', cursor: 'pointer' }}>
            {s <= rating ? '\u2605' : '\u2606'}
          </button>
        ))}
      </div>
      <button onClick={handleThumbsDown} className="text-xs px-2 py-0.5 rounded transition-colors"
        style={{ color: '#7a758066', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.55rem' }}>
        {'\ud83d\udc4e'} No encaja conmigo
      </button>
    </div>
  );
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
      {items.map((item, i) => {
        ...
        return (
          <div key={item.id || i} className="rounded-xl p-5"
            style={{ background: 'rgba(38, 166, 154, 0.04)', border: '1px solid rgba(38, 166, 154, 0.1)' }}>
            <div className="flex items-center gap-2 mb-3">
              <span>{badge}</span>
              <span className="text-xs px-2 py-0.5 rounded-full"
                style={{ background: 'rgba(38, 166, 154, 0.12)', color: '#26a69a', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.6rem' }}>
                {item.type || 'note'}
              </span>
              {item.created_at && (
                <span className="text-xs" style={{ color: '#7a758044', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.55rem' }}>
                  {formatDate(item.created_at)}
                </span>
              )}
            </div>
            <h3 className="text-base font-medium mb-3" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#d4d0cb' }}>
              {item.title}
            </h3>
...
export default function YoshiPage() {
  ...
  const loadMessages = useCallback(async () => {
    try {
      const data = await yoshiFetchMessages();
  ...
  const loadContext = useCallback(async () => {
    try {
      const data = await yoshiFetchContext();
      setContext(Array.isArray(data) ? data : (data.items || []));
    } catch { /* silent */ }
  }, []);
  ...
  useEffect(() => {
    ...
    loadMessages();
    loadContext();
  }, [authed, loadMessages, loadContext]);
  ...
  const handleDeleteContext = async (id) => {
    try {
      await yoshiDeleteContext(id);
      loadContext();
    } catch { /* silent */ }
  };

  const handleFeedback = async (data) => {
    try { await yoshiSendFeedback(data); } catch { /* silent */ }
  };

  if (checking) {
    return <div className="flex items-center justify-center min-h-screen" style={{ background: '#08080d' }}>
      <div className="animate-pulse text-xs" style={{ color: '#7a7580' }}>...</div>
    </div>;
  }

  if (!authed) {
    return <YoshiAuth onAuth={() => setAuthed(true)} />;
  }
  ...
  return (
    <div className="flex h-screen" style={{ background: '#08080d' }}>
        ...
        <header className="flex items-center justify-between px-4 sm:px-6 py-3 border-b border-white/5">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-base font-semibold" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#26a69a' }}>
                Athena
              </span>
            </div>
            <div className="flex gap-1">
              <button onClick={() => setActiveTab('chat')}
                ...
                style={{
                  background: activeTab === 'chat' ? 'rgba(38, 166, 154, 0.12)' : 'transparent',
                  color: activeTab === 'chat' ? '#26a69a' : '#7a7580',
              ...
              <button onClick={() => setActiveTab('library')}
                className="px-3 py-1.5 rounded-lg text-xs transition-colors"
                style={{
                  background: activeTab === 'library' ? 'rgba(38, 166, 154, 0.12)' : 'transparent',
                  color: activeTab === 'library' ? '#26a69a' : '#7a7580',
                  fontFamily: "'DM Sans', sans-serif",
                }}>
                {'\ud83d\udcda'} Library
              </button>
          ...
          <div className="flex items-center gap-2">
            {activeTab === 'chat' && (
              <button onClick={() => setContextOpen(!contextOpen)}
                className="px-2 py-1 rounded text-xs lg:hidden"
                style={{ color: '#7a7580', border: '1px solid rgba(255,255,255,0.06)' }}>
                {contextOpen ? 'Close' : 'Context'}
              </button>
            )}
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4">
              {messages.length === 0 && !sending && (
                <div className="text-center py-12">
                  <div className="text-3xl mb-3">{String.fromCodePoint(0x2600, 0xFE0F)}</div>
                  <p className="text-sm" style={{ color: '#7a7580', fontFamily: "'Cormorant Garamond', serif" }}>
                    This is your private space with Athena.
                  </p>
                  <p className="text-xs mt-1" style={{ color: '#7a758044', fontFamily: "'DM Sans', sans-serif" }}>
                    Say anything. Everything here stays between you two.
                  </p>
                </div>
              )}
              {messages.map((msg, i) => {
                    ...
                    <div className="max-w-lg" style={{ maxWidth: '80%' }}>
                      {isAthena && (
                        <div className="flex items-center gap-1.5 mb-1">
                          ...
                          <span className="text-xs font-semibold" style={{ fontFamily: "'Cormorant Garamond', serif", color: '#26a69a', fontSize: '0.85rem' }}>
                            ATHENA
                          </span>
                          <span className="text-xs" style={{ color: '#7a758033', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.55rem' }}>
                            {formatTime(msg.timestamp)}
                          </span>
                        </div>
                      )}
                      <div className={`rounded-xl px-4 py-3 ${isAthena ? '' : 'ml-auto'}`}
                        style={{
                          background: isAthena ? 'transparent' : 'rgba(255,255,255,0.06)',
                          borderLeft: isAthena ? '2px solid rgba(38, 166, 154, 0.2)' : 'none',
                          paddingLeft: isAthena ? '1rem' : undefined,
                        }}>
              ...
              {sending && (
                <div className="flex justify-start">
                  <div className="flex items-center gap-2 px-4 py-2">
                    <span style={{ fontSize: '0.8rem' }}>{String.fromCodePoint(0x2600, 0xFE0F)}</span>
                    <span className="text-xs animate-pulse" style={{ color: '#26a69a', fontFamily: "'DM Sans', sans-serif" }}>
                      Athena is thinking...
                    </span>
                  </div>
                </div>
            ...
            <div className="border-t border-white/5 px-4 sm:px-6 py-3">
              ...
              <div className="text-center mt-2">
                <button onClick={() => { localStorage.removeItem('yoshi_auth'); window.location.reload(); }}
                  className="text-xs" style={{ color: '#7a758033', fontFamily: "'JetBrains Mono', monospace", fontSize: '0.55rem' }}>
                  Logout
                </button>
              </div>
            </div>
      ...
      {/* Context panel (only in chat tab) */}
      {activeTab === 'chat' && (
        <ContextPanel items={context} onDelete={handleDeleteContext}
          open={contextOpen} onClose={() => setContextOpen(false)} />
      )}
    </div>
  );
}
